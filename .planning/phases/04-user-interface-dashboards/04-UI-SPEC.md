...
## Design Source And Data Integrity Gates

Design source of truth:

- The root `designs/` directory is the authoritative source for Phase 04 visual and interaction behavior.
- Implementations should map each major surface to a corresponding design artifact in `designs/*/code.html` and uphold the `designs/aira_intelligence/DESIGN.md` principles.

Runtime data integrity:

- Production composables in this phase must not ship hardcoded mock score values, static fake narratives, or sample histories.
- Data displayed in Home, metric detail, coach, train, and nutrition routes must come from local repositories/Room flows and derived domain outputs.
- Mock data is only acceptable in `@Preview` blocks or explicit debug-only fixtures.

Verification gates (must pass before phase close):

- No placeholder navigation destinations remain in primary dashboard flows.
- Home top section shows confidence context derived from real state.
- Metric and detail insights are generated from trend/current metrics rather than static copy.
- Nutrition and training lists render persisted local entries, with edit/delete where scoped.
